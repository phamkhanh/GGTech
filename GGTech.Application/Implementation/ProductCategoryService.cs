...
namespace GGTech.Application.Implementation
{
    public class ProductCategoryService : IProductCategoryService
    {
        ...
        {
            return _productCategoryRepository.SelectAll().OrderBy(x => x.ParentId)
        ...
        public List<ProductCategoryViewModel> GetAll(string keyword)
        {
            if (!string.IsNullOrEmpty(keyword))
                return _productCategoryRepository.SelectAll(x => x.Name.Contains(keyword)
        ...
        {
            return _productCategoryRepository.SelectAll(x => x.Status == Status.Active
            && x.ParentId == parentId)
             .ProjectTo<ProductCategoryViewModel>()
             .ToList();
        }
        ...
        {
            return Mapper.Map<ProductCategory, ProductCategoryViewModel>(_productCategoryRepository.SelectById(id));
        }

        public List<ProductCategoryViewModel> GetHomeCategories(int top)
        {
            var query = _productCategoryRepository
                .SelectAll(x => x.HomeFlag == true, c => c.Products)
        ...
        {
            var source = _productCategoryRepository.SelectById(sourceId);
            var target = _productCategoryRepository.SelectById(targetId);
        ...
        {
            var sourceCategory = _productCategoryRepository.SelectById(sourceId);
            sourceCategory.ParentId = targetId;
            _productCategoryRepository.Update(sourceCategory);

            //Get all sibling
            var sibling = _productCategoryRepository.SelectAll(x => items.ContainsKey(x.Id));
